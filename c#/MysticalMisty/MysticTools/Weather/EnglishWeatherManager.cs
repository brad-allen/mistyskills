...
namespace Weather.OpenWeather
{
	...
	public class EnglishWeatherManager
	{
		...
		private readonly Timer _weatherUpdate;
		...
		private readonly string _openWeatherApiAuth;
		private readonly string _openWeatherCountryCode;
		private readonly string _openWeatherCity;
		...
		public EnglishWeatherManager(IRobotMessenger misty, string openWeatherApiAuth, string ipStackApiAuth, string openWeatherCountryCode, string openWeatherCity)
		{
			...
			_weatherUpdate = new Timer(UpdateWeather, null, 0, 5000 * 60 * 5); //update weather info every 5 minutes
		}
			...
			if (_currentWeather?.Overall != null)
			{
				...
				switch (_currentWeather.Overall.ToLower())
				{
					...
					case "clouds":
						...
						break;
					case "mist":
						weatherOptions.Add("The weather is misty today, and even though Misty can be beautiful, you need to be careful!");
						break;
					case "dust":
					...
					case "ash":
						...
						break;
					case "squall":
						weatherOptions.Add("Be ready, weather report says you could expect a sudden storm and lots of wind!");
						break;
					case "tornado":
						weatherOptions.Add("Looks like there may be a tornado warning in the area, be careful!");
						break;
					case "smoke":
					case "haze":
					case "fog":
		...
		private async void UpdateWeather(object timerData)
		{
			try
			{
				if (_weatherService != null)
				{
					...
					_currentWeather = await _weatherService.GetCurrentWeather(_openWeatherCity, _openWeatherCountryCode);
