...
namespace FunnyBone
{
	...
	{
		private readonly WebMessenger _webMessenger = new WebMessenger();
		...
		public async Task<ChuckNorrisJokeFormat> GetChuckNorrisJoke(string [] limitTo = null, string [] exclude = null)
		{
			try
			{
				if (exclude == null)
				{
					exclude = new string[] { "explicit", "political", "religion"};
				}
				//Old URL string url = "http://api.icndb.com/jokes/random";
				...
				if (toExclude)
                {
					url += string.Format("?exclude={0}", string.Join(",", exclude));
				}
				...
				if (toInclude)
				{
					...
					url += string.Format("limitTo={0}", string.Join(",", limitTo));
				}
			...
			{
				return new ChuckNorrisJokeFormat {  Value = "I had trouble getting a Chuck Norris joke.", Id = "0" } ;
			}
		}
	}
}
